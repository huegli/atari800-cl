...
;;; --- The candidate lists mirror tests/test-machine.lisp's
;;; --- *OS-ROM-CANDIDATES* / *BASIC-ROM-CANDIDATES* / *DOS-ATR-CANDIDATES*
;;; --- so a checkout the test suite can find its assets in works here too
;;; --- (dumps in the wild are named both atariosxl.rom and ATARIXL.ROM).

(defparameter *os-rom-names*    '("atariosxl.rom" "ATARIXL.ROM" "atarixl.rom"))
(defparameter *basic-rom-names* '("ataribas.rom" "ATARIBAS.ROM"))
(defparameter *dos-atr-names*   '("dos25.atr" "DOS25.ATR" "dos.atr" "DOS.ATR"
                                  "dos2_5.atr" "dos25s.atr"))
...
(let* ((os     (find-asset "ATARI800_CL_OS_ROM"    *os-rom-names*))
       ...
       (atr    (or (and (first (demo-argv))
                        (probe-file (first (demo-argv))))
                   (find-asset "ATARI800_CL_DOS_ATR" *dos-atr-names*))))
  (unless (and os basic)
    (format *error-output*
