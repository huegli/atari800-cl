...
| Socket library | **usocket** — **must be re-added** to `:depends-on` (it was removed with IPC); also re-add **flexi-streams** for octet encoding. |
...
| --- | --- | --- |
| `src/input.lisp` | `:atari800-cl.input` | Single mutex-guarded `input-state` struct holding current keyboard, joystick (2 ports), console-keys, paddle (×4) state. Setters called from socket reader threads; getters called from the emulator thread when PIA / GTIA / POKEY read their input registers. |
| `src/transport.lisp` | `:atari800-cl.transport` | Thin wrappers over `usocket` for TCP listen/accept/close and Unix-domain listen/accept/close. Any implementation-specific fallback for Unix-domain sockets goes through `:atari800-cl.compat` (see CLAUDE.md invariant). |
...
| `src/compat.lisp` | Add: `current-process-id` (SBCL `sb-posix:getpid`, LispWorks `system:getpid`), `chmod-file path mode` (SBCL `sb-posix:chmod`, LispWorks `system:call-system "/bin/chmod"`), `delete-file-if-exists`, and **mandatory** Unix-domain socket helpers `open-unix-listener` / `accept-unix-client` / `open-unix-client`. Try `usocket` with `:protocol :local` first (works on both SBCL and recent LispWorks); fall back per-implementation if the running usocket build can't bind locally — on SBCL via `sb-bsd-sockets:local-socket`, on LispWorks via an FLI wrapper for `socket(AF_UNIX, SOCK_STREAM, 0)` + `bind/listen/accept` (templates exist in the LispWorks user guide's FLI examples). All `#+sbcl`/`#+lispworks` reader conditionals stay confined here. |
...
- **High-priority commands** (`pause`, `resume`, `reset`, `quit`) set
  ...
  (one scanline) and bails out so the mailbox drains within ~63 µs.
...
10. README/CHANGES updates. *(No `ipc.lisp` deprecation banner — the
    file is gone.)*
11. `main.lisp` re-exports; manual smoke test (§ Verification).

**Each step ends with a green run of the §Verification commands (the
